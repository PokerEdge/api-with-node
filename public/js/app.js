// ~function(){
  ...
  socket.on('new tweet', function( {newTweetData} ){
    ...
    $timeline.prepend(
      // '<div>' + newTweetData.text +
      // ' ' + newTweetData.retweetCount + ' ' + newTweetData.name +
      // '</div>'

      // `li
      //   strong.app--tweet--timestamp #{tweetObject.timeSinceTweet}
      //   a.app--tweet--author
      //     .app--avatar(style=`background-image: url(${userAvatarImage})`)
      //       img(src=`background-image: url(${userAvatarImage})`)
      //     h4 #{name}
      //     |  @#{screen_name}
      //   p #{tweetObject.tweetText}
      ...
      //         strong #{tweetObject.retweetCount}
      //     li
      //       a.app--like
      //         span.tooltip Like
      ...
      //         strong #{tweetObject.likeCount}`
          ...
          <h4>${newTweetData.name}</h4> @${newTweetData.screen_name}</a>
        <p>${newTweetData.text}</p>
        <ul class="app--tweet--actions circle--list--inline">
          <li><a class="app--reply"><span class="tooltip">Reply</span>
              <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewbox="0 0 38 28" xml:space="preserve">
          ...
          <li><a class="app--retweet"><span class="tooltip">Retweet</span>
              ...
              </svg><strong>${newTweetData.retweetCountOfTweet}</strong></a></li>
          <li><a class="app--like"><span class="tooltip">Like</span>
              ...
              </svg><strong>${newTweetData.likeCountOfTweet}</strong></a></li>
        </ul>
      </li>`
